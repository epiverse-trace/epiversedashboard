...
```{r}
valueBox(pr_opened_sum, icon = "fa-exclamation-circle", color = "#cfe2f3")
```

### Pull requests merged / closed
```{r}
valueBox(pr_closed_sum, icon = "ion-merge", color = "#ead1e8")
```

### Pull requests `r duration` change
```{r}
valueBox(pr_change, icon = "fa-exclamation-circle", color = if (pr_change <= 0) "#D1E2CC" else "danger")
```

### Remaining pull requests
```{r}
valueBox(pr_open, icon = "fa-clipboard-list", color = "#DDD")
```

### Issues opened
```{r}
valueBox(issue_opened_sum, icon = "fa-exclamation-circle", color = "#cfe2f3")
```

### Issues closed
```{r}
valueBox(issue_closed_sum, icon = "fa-times-circle", color = "#ead1e8")
```


### Issues `r duration` change
```{r}
valueBox(issue_change, icon = if (issue_change <= 0) "fa-arrow-down" else "fa-arrow-up", color = if (issue_change <= 0) "#D1E2CC" else "danger")
```

### Remaining issues
```{r}
valueBox(issue_open, icon = "fa-clipboard-list", color = "#DDD")
```
